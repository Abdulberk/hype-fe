  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
      ...
      <Paper elevation={1} sx={{ p: 2 }}>
        ...
        <LegendItem
          color={COLOR_SCHEME.myPlace}
          label="My Place"
          description="Primary location"
        />
        
        <LegendItem
          color={COLOR_SCHEME.competitor}
          label="Competitors"
          ...
          {TRADE_AREA_PERCENTAGES.map(percentage => {
            ...
            return (
              <LegendItem
                ...
                description={`Customer catchment area (${opacity} opacity)`}
              />
            );
          })}
          
          <Typography variant="caption" color="text.secondary" sx={{ mt: 1, display: 'block' }}>
            Larger percentages indicate smaller, more concentrated areas.
          </Typography>
        </Paper>
      )}
        ...
        <Paper elevation={1} sx={{ p: 2 }}>
          ...
          {homeZipcodesPercentiles.map((percentile, index) => {
            const percentileRanges = [
              { min: 0, max: 20, label: '0-20th percentile' },
              { min: 20, max: 40, label: '20-40th percentile' },
              { min: 40, max: 60, label: '40-60th percentile' },
              { min: 60, max: 80, label: '60-80th percentile' },
              { min: 80, max: 100, label: '80-100th percentile' }
            ];
            ...
            return (
              <LegendItem
                ...
                label={range.label}
                description={`${percentile.min.toFixed(1)}% - ${percentile.max.toFixed(1)}%`}
              />
            );
          })}
          
          <Typography variant="caption" color="text.secondary" sx={{ mt: 1, display: 'block' }}>
            Percentages represent customer density from each zipcode area.
          </Typography>
        </Paper>
      )}
        ...
        <Paper elevation={1} sx={{ p: 2 }}>
          <Typography variant="body2" color="text.secondary" sx={{ textAlign: 'center' }}>
            {!customerAnalysis.isVisible 
              ...
              ? 'No trade areas selected'
              : 'No home zipcodes data available'
            }
          </Typography>
        </Paper>
      )}
